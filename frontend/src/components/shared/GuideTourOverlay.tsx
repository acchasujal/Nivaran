...
const stepsData: Record<number, TourStepData> = {
  1: {
    targetId: 'intake-form-container',
    title: '1. Evidence Submission',
    explanation: 'Upload a photograph of the civic issue. The AI model extracts incident parameters and validates integrity.',
    whyItMatters: 'Ensures only high-credibility, visually documented issues enter the public pipeline.',
    whatIsThis: 'Photo upload and AI intake.',
    whyIsUseful: 'Automates manual reporting steps.',
    howImproveAccountability: 'Grounds every complaint in hard visual proof.'
  },
  2: {
  ...
  3: {
    ...
    title: '3. Public Transparency Dashboard',
  ...
  5: {
    ...
    title: '5. Cross-Issue Silence Ledger',
  ...
  6: {
    ...
    title: '6. Ward Pattern Intelligence',
  ...
  7: {
    ...
    title: '7. Evidence Integrity Badge',
  ...
  9: {
    ...
    title: '9. Community Verification',
  ...
  12: {
    ...
    title: '12. Government Response Tracker',
  ...
  13: {
    ...
    title: '13. Accountability Timeline',
...
export const GuideTourOverlay: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const updateHighlight = () => {
      ...
      if (element) {
        ...
        // Fallback: clear highlight, card will render in fixed center
        setHighlightStyle(null);
      }
    };
  ...
  if (showWelcome) {
    return (
      <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[9999] animate-fade font-sans">
        <div className="bg-white border border-slate-200 rounded-medium shadow-premium max-w-sm w-full p-6 text-center space-y-6">
          <div className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-primary/10 text-primary mb-2 shrink-0 animate-pulse">
            <Sparkles size={24} />
          </div>
          <div className="space-y-2 select-none">
            <h2 className="text-lg font-bold text-slate-800">Welcome to CivicPulse</h2>
            <p className="text-xs text-slate-500 leading-relaxed">
              Explore the AI-grounded civic operations pipeline. Takes approximately 2–3 minutes to view all capabilities.
            </p>
          </div>
          <div className="flex flex-col gap-2 pt-2">
            <button
              onClick={startTour}
              className="w-full inline-flex items-center justify-center gap-1.5 px-4 py-2.5 bg-primary hover:bg-primary-hover text-white text-xs font-bold rounded-small shadow-sm transition-all cursor-pointer"
            >
              Start Tour
            </button>
            <button
              onClick={skipTour}
              className="w-full inline-flex items-center justify-center gap-1.5 px-4 py-2.5 border border-slate-250 bg-white hover:bg-slate-50 text-slate-700 text-xs font-bold rounded-small transition-all cursor-pointer"
            >
              Skip
            </button>
            <button
              onClick={dontShowAgain}
              className="w-full inline-flex items-center justify-center gap-1.5 px-4 py-2.5 text-slate-400 hover:text-slate-600 text-[10px] font-bold transition-all cursor-pointer"
            >
              Don't show again
            </button>
          </div>
        ...
        <div className="flex items-center justify-between border-t border-slate-100 pt-3 text-xs">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="inline-flex items-center gap-1 px-3 py-1.5 border border-slate-250 bg-white text-slate-700 font-bold rounded hover:bg-slate-50 disabled:opacity-30 disabled:hover:bg-white cursor-pointer"
            >
              <ArrowLeft size={12} />
              <span>Back</span>
            </button>
            ...
            >
              <span>{currentStep === 14 ? 'Finish' : 'Next'}</span>
              <ArrowRight size={12} />
            </button>
          </div>
        </div>
